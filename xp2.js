...
if (!videoheaderArr[0]) {
  ...
  for (let i = 0; i < videoheaderArr.length; i++) {
    ...
    .finally(() => $.done())
function GetCookie() {
if($request&&$request.url.indexOf("give_redbag_by_live")>=0) {
 ...
 if($request&&$request.body.indexOf('"actId":')>=0) {
   const livebody = $request.body
    if(livebody)  $.setdata(livebody,`livebody${$.idx}`)
    $.log(`[${jsname}] 获取livevideo请求: 成功,livebody: ${livebody}`)
    $.msg(`livebody${$.idx}: 成功🎉`, ``)
 }
 }
...
return new Promise((resolve, reject) => {
  let profiturl ={
   ...
   $.get(profiturl,(error, response, data) =>{
     ...
     $.log('xpvideo'+num)
     if(num >= 5){gold = 1}
	 if ((hour >= 7 && minute <= 10) || (hour <= 23 && minute >= 50)) {
		{live = 1} 
	 }
