...
function execTask(ac, i) {
  return new Promise(async resolve => {
    try {
      await $.wait(i * 50)
      await ysm4(ac)
      if ((!execNo || (execNo.length == 0 || execNo.includes(ac.no))) && ac.remain_read && !(ac.day_read < 2 && ysmBanfirstTask) && !(ysmBanhalfTask && ac.day_read == 30&& ac.day_read == 50)) {
      ...
      try {
        if (err) {
          ...
          if (result.errcode == 0 && result.data) {
            ...
            } else if (ysmBanhalfTask && ac.day_read == 30 && ac.day_read == 50) {
              f = 0
              $.msg(`${$.name}: 账号${ac.no}`, '', `今日已阅读30/50篇，请手动阅读2篇再跑脚本`)
            } else {
              f = 1
            }
